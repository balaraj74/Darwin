...
echo -e "${NC}"
echo -e "${CYAN}======================================================${NC}"
echo -e "${BOLD}🧬 Initializing Digital Twin Ecosystem...${NC}\n"
...
if [ -d "backend/venv" ]; then
    ...
    (cd backend && uvicorn main:app --host 0.0.0.0 --port 8000 --reload) > .backend.log 2>&1 &
fi
BACKEND_PID=$!

# 💻 Start Frontend (Next.js)
echo -e "🚀 ${CYAN}Booting Client Interface (Next.js on Port 3000)...${NC}"
